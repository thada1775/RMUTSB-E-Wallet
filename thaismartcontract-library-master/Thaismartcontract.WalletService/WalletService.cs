    ...
    public class WalletService
    {
        ...
        private void UpdateBalance(bool reset = false)
        {
            ...
            if (reset)
            {
                ...
                ledgers = ledger.FindAll().ToList();
            }
            else
            {
                ledgers = ledger.Find(Query.And(Query.EQ("Status", "NotProcessed"), Query.All("Time", Query.Ascending))).ToList();
        ...
        public List<Ledger> GetLedgers(byte[] pubkeyHash, int skip = 0, int limit = int.MaxValue)
        {
            try
            {
                List<Ledger> output = new List<Ledger>();
                var toSkip = skip;
                var ledgers = ledger.Find(Query.All("Time", Query.Descending)).ToList().OrderByDescending(l => l.Time);
                //var ledgers = ledger.FindAll().ToList();
                foreach (var row in ledgers)
                {
                    if (row.TokenReceiverHash.QuickCompare(pubkeyHash) ||
                        row.TokenSenderHash.QuickCompare(pubkeyHash))
                        //if (true)
                    {
                        if (toSkip > 0)
                        {
                        ...
                        {
                            row.TokenName = contract.TokenName;
                            output.Add(row);
                        }
                ...
                return output;
                //var tt = ledger.Find(Query.EQ("TokenReceiverHash", pubkeyHash)).ToList();
                //var testTokenReceiver = ledger.Find(Query.Or(Query.EQ("TokenReceiverHash", pubkeyHash),
                //                                         Query.EQ("TokenSenderHash", pubkeyHash)))
                //    .ToList();
                //var ledgers = ledger.Find(Query.And(Query.All("Time", Query.Descending),
                //                                Query.Or(Query.EQ("TokenReceiverHash", pubkeyHash),
                //                                         Query.EQ("TokenSenderHash", pubkeyHash))))
                //                        .Skip(skip)
                //                        .Take(limit);
        ...
        {
            var pubHashkey = userPrivateKey.GetAddress().Hash.ToBytes();
